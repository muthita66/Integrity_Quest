...
export default function CopyWorkAnimation({
    ...
    const [showStudentReaction, setShowStudentReaction] = useState(false);
    ...
    const handleSkip = () => {
        ...
        setDialogue(
            "ขอไฟล์หน่อย ส่งมาทั้งชุดเลยได้ไหม"
        );
        ...
        >
            ...
            {dialogue === "ในที่สุด งานก็เสร็จสักที" && (
                <motion.div
                    ...
                    className="absolute left-1/2 -translate-x-[90%] top-[20%] bg-white text-black px-5 py-3 rounded-2xl shadow-xl z-40">
                    {dialogue}
                </motion.div>
            )}
            ...
            {showFriendDialogue && (
                <motion.img
                    src={friend}
                    alt=""
                    className="absolute left-[-7%] bottom-[0%] w-85 z-30 scale-x-[-1]"
                    initial={{
                        x: -200,
                        opacity: 0,
                    }}
            ...
            {showStudentReaction && (
                <motion.img
                    src={student5}
                    alt=""
                    className="absolute right-[3%] bottom-[0%] w-75 z-30"
                    initial={{
                        opacity: 0,
                        x: 100,
                    }}
            ...
            {showFriendDialogue &&
                ...
                "ขอไฟล์หน่อย ส่งมาทั้งชุดเลยได้ไหม" && (
                    <motion.div
                        ...
                        className="absolute left-[18%] top-[15%] bg-white text-black px-5 py-3 rounded-2xl shadow-xl z-40">
                        {dialogue}
                    </motion.div>
                )}
